...
**IKPA Solution:** The **Shark Auditor** uses regex pattern matching and anomaly detection to surface zombie subscriptions with **annualized framing** ("₦3,000/month → ₦36,000/year. That's 7% of your rent.") and a **Tinder-style swipe UI** for instant decisions.
...
Full "Cognitive Chain" tracing: Transaction → Metrics → Simulation → LLM → User Decision
...
"91% of financial resolutions fail. Not because people lack data—Mint gave them dashboards. Not because they lack knowledge—YNAB taught them budgeting. They fail because of five specific failure modes. IKPA defeats each one."

**2. Failure Mode 1: Invisible Leakage → Shark Auditor (1min)**
...
"These zombie subscriptions cost ₦240,000/year—that's 12% of her house goal. One swipe to kill each zombie."

[User swipes left on both]
"Total recovered: ₦240,000/year. The Shark Auditor defeated invisible leakage."

**3. Failure Mode 2: Failure Spiral → GPS Re-Router (1min)**
...
"This triggers shame → avoidance → abandonment. The failure spiral. But IKPA's GPS Re-Router shows this instead:"
...
**4. Failure Mode 3: Temporal Disconnect → Future Self Simulator (1min)**
...
"Dear Aisha,
I'm writing this from the balcony of our flat in Victoria Island. Yes, OUR flat—we own it now.
That ₦20,000 you saved in January 2026? It became ₦180,000 by now.
Keep going. I'm proof it works.
Love, Aisha (Age 60)"

"The Future Self Simulator defeated temporal disconnect."

**5. Failure Mode 4: Cultural Blindness → Ubuntu Manager (45s)**

[Show family transfer notification]

"Aisha's mom needs ₦100K for medical bills. Western apps would show this as a budget problem. IKPA's Ubuntu Manager shows this:"

[Show Ubuntu Manager response]
"Family comes first—that's not a bug, it's a feature of who you are."

[Show Dependency Ratio gauge]
"Your dependency ratio: 21% (Healthy Orange Zone)"

[Show adjustment options]
"Use emergency fund, adjust timeline, or reduce this month's savings—all guilt-free."

"The Ubuntu Manager defeated cultural blindness."

**6. Failure Mode 5: Commitment Decay → Commitment Device Engine (45s)**

[Show goal creation screen]

"Aisha sets her goal: Save ₦2M for a house down payment by December. But here's the twist—IKPA asks:"
...
"And Aisha can share her commitment journey—a beautiful story card for social media. Viral growth built into every achievement."
...
"Every agent action—from transaction detection to LLM response to user decision—is traced in Opik."

[Show G-Eval scores]
"We use LLM-as-a-Judge with G-Eval. This measures ToneEmpathy—how supportive our AI sounds."
...
"IKPA doesn't track money—it fights for you.
...
│   │   │
│   │   ├── agents/
│   │   │   ├── shark-auditor.agent.ts      # Subscription detection
│   │   │   ├── gps-rerouter.agent.ts       # Recovery path generation
│   │   │   ├── future-self.agent.ts        # Letter from 2045
│   │   │   ├── ubuntu-manager.agent.ts     # Cultural context
│   │   │   └── commitment-device.agent.ts  # Stakes enforcement
│   │   │
│   │   ├── opik/
│   │   │   ├── opik.service.ts             # Opik client wrapper
│   │   │   ├── metrics/
│   │   │   │   ├── intervention-success.metric.ts
│   │   │   │   ├── tone-empathy.metric.ts   # G-Eval
│   │   │   │   ├── cultural-sensitivity.metric.ts  # G-Eval
│   │   │   │   ├── financial-safety.metric.ts
│   │   │   │   └── stake-effectiveness.metric.ts  # Commitment tracking
│   │   │   └── optimizer/
│   │   │       ├── framing-optimizer.ts     # MetaPromptOptimizer
│   │   │       ├── letter-optimizer.ts      # EvolutionaryOptimizer
│   │   │       └── tool-optimizer.ts        # GEPA
│   │   │
│   │   └── prompts/
│   │       ├── shark-auditor.prompt.ts
│   │       ├── gps-rerouter.prompt.ts
│   │       ├── future-self.prompt.ts
│   │       ├── ubuntu-manager.prompt.ts
│   │       └── commitment-device.prompt.ts
│   │
│   ├── finance/
│   │   ├── calculators/
│   │   │   ├── cash-flow-score.ts
│   │   │   ├── simulation-engine.ts         # Dual-Path Monte Carlo
│   │   │   ├── dependency-ratio.ts
│   │   │   └── subscription-detector.ts
│   │   ├── finance.service.ts
│   │   └── finance.cron.ts                  # Daily calculations
│   │
│   └── story-cards/                         # Addon: Shareable content
│       ├── story-cards.module.ts
│       ├── story-cards.service.ts           # Card generation logic
│       ├── story-cards.controller.ts
│       └── templates/
│           ├── future-self.template.ts      # "I talked to my 60-year-old self"
│           ├── commitment.template.ts       # "I put my money where my goals are"
│           ├── milestone.template.ts        # "Goal Achieved!"
│           └── recovery.template.ts         # "I made a wrong turn. Then I recalculated."
```
